...
const useEditorModule = (columnId: ComputedRef<number>, id: ComputedRef<number>) => {
  ...
  const listRoute = computed(() => ROUTES[NAMES.EMPLOYEE_DRIVER_LIST](columnId.value));
  const viewRoute = (id: number) => ROUTES[NAMES.EMPLOYEE_DRIVER_VIEW](columnId.value, id);
  const cancelRoute = computed(() => (id.value ? viewRoute(id.value) : listRoute.value));

  const loading = ref(false);
  const model = reactive<Driver>({
  ...
  const save = async () => {
    ...
    return dialogs.confirm(t('dialogs.saveConfirm'), () => {
      ...
      const promise = id.value
        ? api.drivers._id.put(id.value, model).then(() => id.value)
        : api.drivers._id.post(model);
      return promise
        .then((id) => {
  ...
  const fetch = () => {
    loading.value = true;
    return api.drivers._id
      .get(id.value)
      .then((row: Driver) => Object.assign(model, row))
...
const useListModule = (columnId: ComputedRef<number>) => {
  ...
  const creatorRoute = computed(() => ROUTES[NAMES.EMPLOYEE_DRIVER_CREATOR](columnId.value));
  const viewRoute = (id: number) => ROUTES[NAMES.EMPLOYEE_DRIVER_VIEW](columnId.value, id);
...
const useViewModule = (columnId: ComputedRef<number>, id: ComputedRef<number>) => {
  ...
  const editorRoute = computed(() =>
    ROUTES[NAMES.EMPLOYEE_DRIVER_EDITOR](columnId.value, id.value)
  );
  const listRoute = computed(() => ROUTES[NAMES.EMPLOYEE_DRIVER_LIST](columnId.value));
